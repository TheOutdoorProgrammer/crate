...
**Read this before `PLAN.md`.** The audit found bugs *in the plan* and bugs *in shipped v1 code*,
and the second category is worth acting on regardless of whether v2 ever happens.
...
| # | Bug | Where |
| --- | --- | --- |
| B12 | **Unknown bitrate scores as top tier.** `QualityTierRank` guards on `bitrate > 0`, so a 96kbps MP3 with an unparseable header satisfies every tier's minimum and is marked *not* upgradeable. Also: CLAUDE.md's stated reason for `mp3info.go` is backwards — the field preventing a full-library re-download is `download_format`, not bitrate. | `downloader/service.go:733,744-746` |
...
| B17 | **Reading one Vorbis comment pulls the entire FLAC through RAM.** `flac.ParseFile` → `ReadAll`. `ParseMetadata` exists and reads only metadata blocks. **This also fixes issue #12** — the "frames do not begin with sync code" error comes from the post-`ReadAll` assertion `ParseMetadata` never runs. | `importer/tags.go:98` |
| B18 | **`IsHealthy` holds `mu.RLock` across a 2s network call**, and `ListProviders` loops it serially on every Settings/Search/ArtistDetail load. ~4s today with 2 providers; ~14s with v2's seven. A hung provider blocks re-registration. | `manager.go:98-133` |
...
| B26 | **`TestScoringBalance` does not test the production scoring path.** It re-implements the formula inside the test, hardcoding the `+20` and `+10` literals. It would survive the v2 refactor unchanged **and** keep passing while `scoreCandidates` silently inverted. | `service_test.go:523-534` |
...
| X4 | **`external` providers per kind.** **Settled: all first-party providers ship in the same Docker image**, so filesystem locality is preserved by construction and nothing needs to change — the repo split is a *source* split, not a runtime split. The constraint matters for **external** providers, who must be told: catalog and notification are pure network clients and can run anywhere; download works with a mount caveat; **ingest cannot** — `Place` takes a path, and a remote provider stat-ing `/app/downloads/x.flac` either finds nothing or finds a *different* file. Crate should refuse to register an external ingest provider, loudly, at startup. This also needs to be written into `PROVIDERS.md`, which today advertises `external:host:port` with no caveat at all. |
